...
inputs/outputs in a Postgres registry, and provides a small
...
A Postgres registry is the source of truth for run state, aliases,
eval-request dedup and pipelines; the filesystem holds the bulk data
that runs produce. Each user's runs land at
`runs_base/runs/<user>/<run_id>/.lab/` and artifacts at
`<artifact_root>/<kind>/<user>/<alias>/`. The CLI is the only writer:
`labctl run` opens the registry directly, snapshots the source repo,
renders the sbatch script, and shells out to `sbatch` under its own
uid. A per-user `labctl agent` runs reconcile + evald + throttle as a
systemd unit. `labctl serve` is a read-only HTTP server over the same
registry.
