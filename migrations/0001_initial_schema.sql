...
--
-- ============================================================
-- DO NOT EDIT THIS FILE AFTER ROLLOUT.
-- ============================================================
-- sqlx::migrate! tracks each migration's content checksum in
-- `_sqlx_migrations`. Any edit to this file changes its checksum,
-- and every deployed DB's startup will then refuse to launch
-- with: "error returned from database: ... 0001 changed". There
-- is no safe in-place patch path post-rollout.
--
-- For ANY schema change after deploy: add a new
-- `0002_<description>.sql` (and 0003, 0004, …). sqlx applies them
-- in order. The new migration writes its diff against the schema
-- this file declares — never edit 0001 retroactively.
-- ============================================================
--
-- Conventions:
...
CREATE TABLE runs (
    ...
    -- Two accepted recipe_hash shapes: the canonical 64-char sha256
    -- hex digest, and the legacy `synth_<56hex>` form used by the
    -- synthetic-data generators. Anything else is a writer bug.
    CONSTRAINT runs_recipe_hash_format CHECK (
        recipe_hash ~ '^[0-9a-f]{64}$'
        OR recipe_hash ~ '^synth_[0-9a-f]{56}$'
    ),
    -- dependency_on is `{"afterok": [...]}` or NULL. Catches drift in
    -- the only writer (submit_pipeline) at INSERT time.
    CONSTRAINT runs_dependency_on_shape CHECK (
        dependency_on IS NULL OR (
            jsonb_typeof(dependency_on) = 'object'
            AND jsonb_typeof(dependency_on->'afterok') = 'array'
        )
    ),
...
CREATE TABLE eval_requests (
    ...
    CONSTRAINT eval_requests_recipe_hash_format
        CHECK (eval_recipe_hash ~ '^[0-9a-f]{64}$'),
...
CREATE TABLE events (
    ...
    -- Every emitter builds the payload via serde_json::json!({...}) so
    -- this is the structural floor — scalars / arrays slipping in are
    -- writer bugs and should be loud.
    CONSTRAINT events_payload_object CHECK (jsonb_typeof(payload_json) = 'object')
);
